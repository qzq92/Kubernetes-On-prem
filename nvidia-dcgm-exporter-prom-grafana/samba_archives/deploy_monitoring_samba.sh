...
echo "Creaing monitoring-gpu namespaces"
kubectl create ns monitoring-gpu

helm install ingress-controller ./nginx-controller/nginx-ingress-0.6.1.tgz --set controller.service.loadBalancerIP=192.168.111.45

echo "Creating grafana secret, pv,pvc using samba share"
kubectl create secret generic grafana-password --from-literal=adminUser=ioniqadmin --from-literal=adminPassword=admin --namespace monitoring-gpu
kubectl apply -f prometheus-dcgm-samba-pv.yaml -n monitoring-gpu
kubectl apply -f grafana-dcgm-samba-pv.yaml -n monitoring-gpu
...
kubectl apply -f grafana-svc.yaml -n monitoring-gpu
